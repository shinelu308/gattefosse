...
(async () => {
  ...
  ws.on('message', raw => { const m = JSON.parse(raw); if (m.method === 'Runtime.exceptionThrown') { const d = m.params.exceptionDetails; console.log('[EXCEPTION]', d.text, ((d.exception||{}).description||'').substring(0,400)); } if (m.method === 'Runtime.consoleAPICalled' && ['error','warning'].includes(m.params.type)) { console.log('[console.'+m.params.type+']', m.params.args.map(a => a.value !== undefined ? a.value : (a.description||'')).join(' ').substring(0,400)); } });
