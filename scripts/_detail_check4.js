...
(async () => {
  ...
  const dbg = await send(ws, ++id, 'Runtime.evaluate', {
    ...
    expression: `(async () => {
      const out = {};
      out.gatteApiBaseUrl = (typeof GatteAPI !== 'undefined') ? GatteAPI.BASE_URL : 'GatteAPI undefined';
      const r = await fetch('/static/js/api.js');
      const txt = await r.text();
      out.apiJsFirst100 = txt.substring(0, 200);
      out.hasEmptyBase = txt.includes("BASE_URL = ''");
      return out;
    })()`,
  });
