...
(async () => {
  ...
  const dbg = await send(ws, ++id, 'Runtime.evaluate', {
    returnByValue: true,
    expression: `({ base: (typeof GatteAPI!=='undefined')?GatteAPI.BASE_URL:'undef', apiHash: [...document.scripts].filter(s=>s.src.includes('api.js')).map(s=>s.src)[0] })`,
  });
  console.log('API BASE:', JSON.stringify(dbg.result.value));
  const shot
